var site_url = '';
var nav  = [ '#data-pegawai', '#data-absensi', '#data-tkpkn', '#laporan-absensi', '#laporan-ketidakhadiran', '#load-data', '#user-setting', '#logout' ];
						...
						$(this)
							.children()
								.animate( {backgroundPosition:'(0px -45px)'},"fast",function(){
											...
											$(parent).css({backgroundImage: 'url(asset/img/nav.png)'});
									});
					});
			break;
		}
...
{
	for ( i = 0; i < nav.length; i++ ){
		$(nav[i]).css({backgroundPosition: i*(-100) + 'px -25px'});
...
function mMouseOver(e)
{
	...
	$(this)
		...
		.css({backgroundImage: 'url(asset/img/nav-over.png)',cursor: 'pointer'})
...
{			
	...
	$(this)
		...
		.css({backgroundImage: 'url(asset/img/nav.png)', cursor: ''});
}

function mClick(e)
{
	location.href = 'index.php?mod='+this.id;
}
