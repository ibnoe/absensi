...
function laporan_ketidakhadiran()
{
	...
	$sql = "SELECT fsidno, fsname, COUNT(tanggal),
				   ...
				   SUM(CASE WHEN masuk_diff < 1 THEN 0
						...
						WHEN (masuk_diff > 91 
								AND keterangan NOT IN ('DL', 'CT', 'TB', 'ST', 'SC', 'I', 'CBL', 'CB', 'CP', 'CS', 'SC', 'BT')
								AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) )
							OR keterangan = 'TL.4' THEN 2.5 END) as masuk,
					SUM(CASE WHEN keluar_diff < 1 THEN 0
						...
						WHEN (keluar_diff > 91 
								AND keterangan NOT IN ('DL', 'CT', 'TB', 'ST', 'SC', 'I', 'CBL', 'CB', 'CP', 'CS', 'SC', 'BT')
								AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) )
							OR keterangan = 'PSW.4' THEN 2.5 END) as keluar
			FROM (
			...
			WHERE UNIX_TIMESTAMP(a.tanggal) >= ".$sdate." AND UNIX_TIMESTAMP(a.tanggal) <=".$edate."
	...
	if($data!="null")
	{
		...
		if($_GET['nama_pjs'] || $_GET['nip_pjs'])
		{
			$nama_pjs = $_GET['nama_pjs'];
			$nip_pjs = $_GET['nip_pjs'];	
		...
		else
		{
			...
			$nama_pjs = $penandatangan[0]['nama'];
			$nip_pjs = $penandatangan[0]['nip'];
		}
		$output .= '<table cellspacing="-1" align="center" width="980px">';
		$output .= '<tr>';
		$output .= '<td width="60%">&nbsp;</td>';
		$output .= '<td width="40%">'.$jabatan.'</td>';
		$output .= '</tr>';
		$output .= '<tr>';
		...
		$output .= '<tr>';
		$output .= '<td>&nbsp;</td>';
		$output .= '<td>'.$nama_pjs.'</td>';
		$output .= '</tr>';
		$output .= '<tr>';
		$output .= '<td>&nbsp;</td>';
		$output .= '<td>NIP. '.$nip_pjs.'</td>';
		$output .= '</tr>';
		$output .= '</table>';
	}
	//echo $output;
...
function laporan_pegawai()
{
	...
	if($data!="null")
	{
		...
		$output .= '</tr>';
		$output .= '<tr>';
		$output .= '<td align="left">Unit: '.$data[0]['eselon_2'].'</td>';
		$output .= '</tr>';
		$output .= '<tr>';
...
function laporan_absensi()
{
	...
	if($data!="null")
	{
		...
		for($i=0; $i<$totalRow; $i++)
		{
			...
			$output .= '<td class="tableRow name" width="100px">'.$pegawaiNama[$i].'<br>'.$pegawaiNIP[$i].'</td>';
			foreach($fieldTanggal as $tanggal)
			{
				foreach($data as $value)
				{
					...
					if($value['tanggal'] == $tanggal && $value['fsidno'] == $pegawaiNIP[$i])
					{
						...
						if( $hari != "Sun" && $hari != "Sat" && !in_array(trim($tanggal), $hariLibur) )
						{
							...
							else
							{
								...
								if ($tkpkn==0 && ($value['keterangan']!="" || in_array($value['keterangan'], $ketTanpaPotongan)) )
								{
									$tanpaPotongan++;
								}
								else
...
function daftar_absensi()
{
	...
	if($data!="null")
	{
		...
		for($i=0; $i<$totalPage; $i++)
		{
			...
			$output .= '</tr>';
			$output .= '<tr align="center">';
			$output .= '<td colspan="2">DAFTAR ABSENSI PEGAWAI</td>';
			$output .= '</tr>';
			$output .= '<tr align="center">';
			$output .= '<td colspan="2">UNIT: '.$unit[0]['nama_id'].'</td>';
			$output .= '</tr>';
			$output .= '<tr align="center">';
			$output .= '<td colspan="2">PERIODE: '.$_GET['tgl_selesai'].'</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td width="350px">Jumlah Pegawai Seluruhnya</td>';
			$output .= '<td>: '.$totalRow.'</td>';
