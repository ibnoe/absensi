...
switch ($_GET['f'])
{
	...
	case "getSetRangeAbsen":
		getSetRangeAbsen();
		break;
}

function getSetRangeAbsen()
{
	global $db;
	$sql = "SELECT 	c.userid as CardNo, 
					a.nip as nip, 
					a.nama as nama,
					b.keterangan, 
					b.tgl_mulai, 
					b.tgl_akhir		
			FROM pegawai a
			LEFT JOIN range_cuti b ON a.nip = b.fsidno
			join pegawai_registry c ON a.nip = c.nip
			WHERE a.nip = '".$_POST['nip']."'
			ORDER BY id DESC";
			
	$result = $db->dbFetchArray($sql);
	$tgllMulai = ($result[0]['tgl_mulai']==NULL) ? $tgllMulai=date("d-m-Y", mktime(0,0,0,date("m"), date("d")-1, date("Y"))) : 
	$tgllMulai=substr($result[0]['tgl_mulai'],8,2)."-".substr($result[0]['tgl_mulai'],5,2)."-".substr($result[0]['tgl_mulai'],0,4);
	$tgllAkhir = ($result[0]['tgl_akhir']==NULL) ? $tgllMulai=date("d-m-Y", mktime(0,0,0,date("m"), date("d")-1, date("Y"))) :
	$tgl_akhir=substr($result[0]['tgl_akhir'],8,2)."-".substr($result[0]['tgl_akhir'],5,2)."-".substr($result[0]['tgl_akhir'],0,4);
	$selCT="";
	$selTB="";
	if ($result[0]['keterangan']=="CT"){$selCT='Selected="selected"';} else {$selCT="";};
	if ($result[0]['keterangan']=="TB"){$selTB='Selected="selected"';} else {$selTB="";};
	if ($result[0]['keterangan']=="DL"){$selDL='Selected="selected"';} else {$selDL="";};
	if ($result[0]['keterangan']=="ST"){$selST='Selected="selected"';} else {$selST="";};
	if ($result[0]['keterangan']=="I"){$selI='Selected="selected"';} else {$selI="";};
	if ($result[0]['keterangan']=="CBL"){$selCBL='Selected="selected"';} else {$selCBL="";};
	if ($result[0]['keterangan']=="CB"){$selCB='Selected="selected"';} else {$selCB="";};
	if ($result[0]['keterangan']=="CP"){$selCP='Selected="selected"';} else {$selCP="";};
	if ($result[0]['keterangan']=="CS"){$selCS='Selected="selected"';} else {$selCS="";};
	if ($result[0]['keterangan']=="SC"){$selSC='Selected="selected"';} else {$selSC="";};
	$output = '<table>
				<tr>
					<td>Nama Pegawai</td>
					<td>:'.$result[0]['nama'].'</td>
				...
				<tr>
					<td>Set Periode </td><td>:
						<input type="text" id="tanggalMulai" size="14" value="'. 
						$tgllMulai.'" /> 
					</td>
					<td>Sampai </td><td>
						<input type="text" id="tanggalAkhir" size="14" value="'. 
						$tgl_akhir.'" /> 
					</td>
				</tr>
				<tr>
					...
					<td colspan="2" align="left">: ';
							$output .= '<select id="keterangan">';
							$output .= '<option value="CT" '.$selCT.'>CT</option>';
							$output .= '<option value="TB" '.$selTB.'>TB</option>';
							$output .= '<option value="DL" '.$selDL.'>DL</option>';
							$output .= '<option value="ST" '.$selST.'>ST</option>';
							$output .= '<option value="I" '.$selI.'>I</option>';
							$output .= '<option value="CBL" '.$selCBL.'>CBL</option>';
							$output .= '<option value="CB" '.$selCB.'>CB</option>';
							$output .= '<option value="CP" '.$selCP.'>CP</option>';
							$output .= '<option value="CS" '.$selCS.'>CS</option>';
							$output .= '<option value="SC" '.$selSC.'>SC</option>';
			$output .=		'</td>
				</tr>
				<tr>
					<td colspan="2" align="center">
						<input type="button" id="btnSimpanRangeAbsen" value="Simpan Data" onclick="simpanRangeAbsen(\''.$result[0]['nip'].'\')" />&nbsp;
						<input type="button" id="btnBatal" value="Batal" onclick="$(\'#SetRangeAbsenForm\').dialog(\'close\')" />
						<input type="hidden" id="CardNo" value="'.$result[0]['CardNo'].'"/>
					</td>
				</tr>
...
function getPegawai()
{
	...
	$eselon_3 = ($_POST['eselon_3'] != "") ? " AND unit_bagian = '".$_POST['eselon_3']."' " : "";
	...
	$sql = "SELECT a.nip, nama, 
			...
			WHERE unit_biro = '".$_POST['eselon_2']."' ".$eselon_3."
			".$where." 
...
function getHariLibur()
{
	...
	$sql = "SELECT id, DATE_FORMAT(tanggal, '%d-%m-%Y') as tanggal, keterangan 
			FROM libur_nasional 
			ORDER BY UNIX_TIMESTAMP(tanggal) DESC";
	...
	foreach($result as $row)
	{
		...
		$output .= '<tr align="left" class="'.$rowClass.'" id="row_'.$rowNumber.'" onclick="selectRow(\'row_'.$rowNumber.'\', \''.$row['id'].'\')">
						<td class="gridrow">'.$rowNumber.'</td>
						<td class="gridrow">'.indonesianDateFormat($row['tanggal'], '-').'</td>
...
function getCutiBersama()
{
	...
	{
		$rowClass = ($rowNumber%2==0) ? "rowEven" : "";
		$output .= '<tr align="left" class="'.$rowClass.'" id="row_'.$rowNumber.'" onclick="selectRow2(\'row_'.$rowNumber.'\', \''.$row['id'].'\')">
						<td class="gridrow">'.$rowNumber.'</td>
						<td class="gridrow">'.indonesianDateFormat($row['tanggal'], '-').'</td>
						<td class="gridrow">'.$row['keterangan'].'</td>
					</tr>';
	...
	$output .= '<tr>
					<td colspan="3" class="gridfooter">
						<div class="button goFirst" onclick="goToPage2(1)">&nbsp;</div>
						<div class="button goPrev" onclick="goToPage2('.$prev.')">&nbsp;</div>
						<div class="pageInfo">Halaman '.$page.' dari total '.$totalPage.' halaman</div>
						<div class="button goNext" onclick="goToPage2('.$next.')">&nbsp;</div>
						<div class="button goLast" onclick="goToPage2('.$totalPage.')"">&nbsp;</div>
					</td>
				</tr>';
	echo $output;
}
...
function getAbsensi()
{
	...
	$sqlLibur = "SELECT DATE_FORMAT(tanggal, '%d-%m-%Y') as tanggal FROM libur_nasional WHERE DATE_FORMAT(tanggal, '%Y') = '".date("Y")."'";
	$libur = $db->dbFetchArray($sqlLibur);
	$tglLibur = array();
	foreach($libur as $tgl)
	{
		$tglLibur[] = $tgl['tanggal'];	
	}
	
	if(in_array($_POST['tanggal'], $tglLibur))
	{
		die('<tr id="loader"><td colspan="4">Hari Libur Nasional</td></tr>');
	}
	...
	$sql = "SELECT c.fscardno, b.userid, a.nip, a.nama, 
			...
			WHERE DATE_FORMAT(c.tanggal, '%d-%m-%Y') = '".$tanggal."' 
			AND unit_biro = '".$_POST['eselon_2']."' ".$eselon_3."
			".$where." 
	...
	if($result=="null")
	{
		die('<tr id="loader"><td colspan="4">Data Tidak Ada.</td></tr>');	
	}
	foreach($result as $row)
	{
		$rowClass = ($rowNumber%2==0) ? "rowEven" : "";
...
function laporan_ketidakhadiran()
{
	...
	$start_date = date('Y-m-d', strtotime($_GET['tgl_mulai']));
	$end_date = date('Y-m-d', strtotime($_GET['tgl_selesai']));
	
	$unit_kerja = ($_GET['unit_bagian'] != 0) ? " d.unit_bagian = '".$_GET['unit_bagian']."'" : " d.unit_biro = '".$_GET['unit_biro']."'";
	...
	$sql = "SELECT fsidno, fsname, COUNT(tanggal),
				   ...
				   golongan,
				   SUM(CASE WHEN keterangan = 'TL.1' OR (masuk_diff > 1 AND masuk_diff < 30) AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) THEN 1 ELSE 0 END) as tl1,
				   SUM(CASE WHEN keterangan = 'TL.2' OR (masuk_diff > 31 AND masuk_diff < 60) AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) THEN 1 ELSE 0 END) as tl2,
				   SUM(CASE WHEN keterangan = 'TL.3' OR (masuk_diff > 61 AND masuk_diff < 90) AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) THEN 1 ELSE 0 END) as tl3,
				   SUM(CASE WHEN (keterangan = 'TL.4' AND tanggal NOT IN (SELECT tanggal FROM libur_nasional)) OR masuk_diff > 91 THEN 1 ELSE 0 END) as tl4,
				   SUM(CASE WHEN keterangan = 'PSW.1' OR (keluar_diff > 1 AND keluar_diff < 30) AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) THEN 1 ELSE 0 END) as psw1,
				   SUM(CASE WHEN keterangan = 'PSW.2' OR (keluar_diff > 31 AND keluar_diff < 60) AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) THEN 1 ELSE 0 END) as psw2,
				   SUM(CASE WHEN keterangan = 'PSW.3' OR (keluar_diff > 61 AND keluar_diff < 90) AND tanggal NOT IN (SELECT tanggal FROM libur_nasional) THEN 1 ELSE 0 END) as psw3,
				   SUM(CASE WHEN (keterangan = 'PSW.4' AND tanggal NOT IN (SELECT tanggal FROM libur_nasional)) OR keluar_diff > 91 THEN 1 ELSE 0 END) as psw4,
				   nama_id
			FROM (
			...
			WHERE UNIX_TIMESTAMP(a.tanggal) >= UNIX_TIMESTAMP('".$start_date."') 
			AND UNIX_TIMESTAMP(a.tanggal) <= UNIX_TIMESTAMP('".$end_date."')
			AND ".$unit_kerja."
			AND tanggal NOT IN (SELECT tanggal FROM libur_nasional)
			AND DATE_FORMAT(tanggal, '%a') NOT IN ('Sun', 'Sat')
	...
	if($data!="null")
	{
		...
		$output .= '.tableRow{border:1px solid; text-align:center; vertical-align:top; padding:2px; font-size:11px;}';
		...
		for($i=0; $i<$totalPage; $i++)
		{
			...
			$output .= '<table align="center" width="1100px"">';
			$output .= '<tr>';
			if($i==0)
			{
			...
			{
				$output .= '<td style="padding:5px;">&nbsp;</td>';	
			}
			...
			if($i==0)
			{
				...
				$output .= '</tr>';
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">UNIT: '.strtoupper($data[0]['nama_id']).'</td>';
				$output .= '</tr>';
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">Periode: '.indonesianDateFormat($_GET['tgl_mulai'], '-').' s/d '.indonesianDateFormat($_GET['tgl_selesai'], '-').'</td>';
				$output .= '</tr>';
				$output .= '<tr>';
				$output .= '<td>Jumlah Pegawai Seluruhnya: '.$totalRow.'</td>';
				$output .= '<td></td>';
				$output .= '</tr>';
			}
			$output .= '</table><br><br>';
			...
			$output .= '<table cellspacing="-1" align="center" width="1100px">';
			$output .= '<tr>';
			...
			$output .= '<td colspan="5" rowspan="2" class="tableHeader">TL</td>';
			$output .= '<td colspan="5" rowspan="2" class="tableHeader">PSW</td>';
			...
			$output .= '<td class="tableHeader">26</td>';
			$output .= '<td class="tableHeader">27</td>';
			$output .= '<td class="tableHeader">28</td>';
			$output .= '<td class="tableHeader">29</td>';
			$output .= '</tr>';
			...
			for($j=0; $j<$rowPerPage; $j++)
			{
				...
				{
					$jumlahTidakhadir = $data[$rowIndex]['tidakhadir']+$data[$rowIndex]['st']+$data[$rowIndex]['i']+$data[$rowIndex]['cbl']+$data[$rowIndex]['cb']+$data[$rowIndex]['cp']+$data[$rowIndex]['cs']+$data[$rowIndex]['sc']+$data[$rowIndex]['bt'];
					$jumlahTanpaPotongan = $data[$rowIndex]['dl']+$data[$rowIndex]['ct']+$data[$rowIndex]['tb'];
					$persenTidakHadir = $jumlahTidakhadir * 5;
					
					$persenTL1 = $data[$rowIndex]['tl1'] * 0.5;
					$persenTL2 = $data[$rowIndex]['tl2'] * 1;
					$persenTL3 = $data[$rowIndex]['tl3'] * 1.25;
					$persenTL4 = $data[$rowIndex]['tl4'] * 2.5;
					
					$persenPSW1 = $data[$rowIndex]['psw1'] * 0.5;
					$persenPSW2 = $data[$rowIndex]['psw2'] * 1;
					$persenPSW3 = $data[$rowIndex]['psw3'] * 1.25;
					$persenPSW4 = $data[$rowIndex]['psw4'] * 2.5;
					
					$persenTL = $persenTL1+$persenTL2+$persenTL3+$persenTL4;
					$persenPSW = $persenPSW1+$persenPSW2+$persenPSW3+$persenPSW4;
					
					$totalTKPKN = $persenTL+$persenPSW+$persenTidakHadir;
					$totalTKPKN = ($totalTKPKN>100) ? 100 : $totalTKPKN;
					
					$output .= '<tr>';
					$output .= '<td class="tableRow">'.($rowIndex+1).'</td>';
					$output .= '<td class="tableRow name" style="width:200px; padding-bottom:8px;">'.$data[$rowIndex]['fsname'].' '.$data[$rowIndex]['jabatan'].'<br>'.$data[$rowIndex]['fsidno'].'</td>';
					$output .= '<td class="tableRow">'.$data[$rowIndex]['golongan'].'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['dl']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['ct']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tb']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($jumlahTanpaPotongan).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tidakhadir']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['st']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['sc']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['i']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['cbl']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['cb']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['cp']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['cs']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['sc']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['bt']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($jumlahTidakhadir).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tl1']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tl2']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tl3']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tl4']).'</td>';
					$totalTL = $data[$rowIndex]['tl1']+$data[$rowIndex]['tl2']+$data[$rowIndex]['tl3']+$data[$rowIndex]['tl4'];
					$output .= '<td class="tableRow">'.convertZero($totalTL).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['psw1']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['psw2']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['psw3']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['psw4']).'</td>';
					$totalPSW = $data[$rowIndex]['psw1']+$data[$rowIndex]['psw2']+$data[$rowIndex]['psw3']+$data[$rowIndex]['psw4'];
					$output .= '<td class="tableRow">'.convertZero($totalPSW).'</td>';
					$output .= '<td class="tableRow">'.$totalTKPKN.'</td>';
					$output .= '</tr>';
				}
		...
		$output .= '<br><br>';
		$output .= genPenandatangan($_GET['nama_pjs'], $_GET['nip_pjs']);
	}
	//echo $output;
...
function laporan_uangmakan()
{
	...
	$sql = "SELECT fsidno, fsname, COUNT(tanggal),
			...
			LEFT JOIN t_par_unitkerja f ON d.unit_bagian = f.c_kode
			LEFT JOIN t_par_subunitkerja g ON d.unit_subbagian = g.c_kode 
			LEFT JOIN t_par_eselon h ON d.golongan = h.c_kode 
			WHERE UNIX_TIMESTAMP(a.tanggal) >= UNIX_TIMESTAMP('".$start_date."') 
			AND UNIX_TIMESTAMP(a.tanggal) <= UNIX_TIMESTAMP('".$end_date."')
			AND ".$unit_kerja."
			AND tanggal NOT IN (SELECT tanggal FROM libur_nasional)
	...
	if($data!="null")
	{
		...
		for($i=0; $i<$totalPage; $i++)
		{
			...
			{
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">LAPORAN BULANAN KETERTIBAN PEGAWAI</td>';
				$output .= '</tr>';
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">UNIT: '.strtoupper($data[0]['nama_id']).'</td>';
				$output .= '</tr>';
			...
			for($j=0; $j<$rowPerPage; $j++)
			{
				...
				if($rowIndex<$totalRow)
				{
					$jumlahTidakhadir = $data[$rowIndex]['tidakhadir']+$data[$rowIndex]['st']+$data[$rowIndex]['i']+$data[$rowIndex]['cbl']+$data[$rowIndex]['cb']+$data[$rowIndex]['cp']+$data[$rowIndex]['cs']+$data[$rowIndex]['sc']+$data[$rowIndex]['dl']+$data[$rowIndex]['ct']+$data[$rowIndex]['tb']+$data[$rowIndex]['bt'];
					
					$output .= '<tr>';
					$output .= '<td class="tableRow">'.($rowIndex+1).'</td>';
					$output .= '<td class="tableRow name" style="padding-bottom:8px;">'.$data[$rowIndex]['fsname'].'<br>'.$data[$rowIndex]['fsidno'].'</td>';
					$output .= '<td class="tableRow">'.$data[$rowIndex]['golongan'].'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['dl']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['ct']).'</td>';
					$output .= '<td class="tableRow">'.convertZero($data[$rowIndex]['tb']).'</td>';
...
function laporan_pegawai()
{
	...
	$sql = "SELECT DATE_FORMAT(a.tanggal, '%d %M %Y') as tanggal, 
			...
			AND UNIX_TIMESTAMP(a.tanggal) <= UNIX_TIMESTAMP('".$end_date."')
			AND b.nip = '".$_GET['nip']."'
			ORDER BY UNIX_TIMESTAMP(a.tanggal)";
	//die($sql);
	$data = $db->dbFetchArray($sql);
	...
	if($data!="null")
	{
		...
		#report style start here
		$output .= '<style>';
		$output .= '.tableHeader{font-weight:bold; border:1px solid; text-align:center; vertical-align:middle; font-size:12px;}';
		$output .= '.tableRow{border:1px solid; text-align:center; vertical-align:top; padding:2px; font-size:12px;}';
		$output .= '.name{text-align:left;}';
		$output .= '</style>';
		#report style end here
		
		#report title start here
		$output .= '<table align="center" width="90%" style="margin-top:25px;">';
		$output .= '<tr>';
		if($i==0)
		{
			$output .= '<td style="padding:5px; font-size:10px;">
						KEMENTERIAN KEUANGAN REPUBLIK INDONESIA<br>
						DIREKTORAT JENDERAL KEKAYAAN NEGARA
						</td>';
		}
		...
		$output .= '<td style="padding:5px; text-align:right; font-size:10px;">
					Tanggal: '.date('d-m-Y').'<br>
					Halaman: '.($i+1).'
					</td>';
		$output .= '</tr>';
		if($i==0)
		{
			...
			$output .= '<tr align="center">';
			$output .= '<td colspan="2">';
			$output .= '<table align="center" width="50%">';
			$output .= '<tr>';
			$output .= '<td align="left" width="35px">Nama</td>';
			$output .= '<td width="5px">: </td>';
			$output .= '<td align="left">'.$data[0]['fsname'].'</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td align="left">NIP</td>';
			$output .= '<td width="5px">: </td>';
			$output .= '<td align="left">'.$data[0]['fsidno'].'</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td align="left">Pangkat/Gol</td>';
			$output .= '<td width="5px">: </td>';
			$output .= '<td align="left">'.$data[0]['v_pangkatgol'].'</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td align="left">Unit</td>';
			$output .= '<td width="5px">: </td>';
			$output .= '<td align="left">'.$data[0]['eselon_2'].'</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td align="left">&nbsp;</td>';
			$output .= '<td width="5px"></td>';
			$output .= '<td align="left">'.$data[0]['eselon_3'].'</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td align="center" colspan="3">Periode: '.indonesianDateFormat($_GET['tgl_mulai'], '-').' s/d '.indonesianDateFormat($_GET['tgl_selesai'], '-').'</td>';
			$output .= '</tr>';
		}
		$output .= '</table>';
		$output .= '</td>';
		...
		foreach($data as $value)
		{
			...
			$output .= '<td class="tableRow" align="left">'.$uraian.'</td>';
			$output .= '</tr>';
			$num++;
		}
		$output .= '</table>';
...
function indonesianDateFormat($strDate, $dateSeparator)
{
	$arrData = explode($dateSeparator, $strDate);
	...
	return $arrData[0]." ".$arrBulan[$arrData[1]]." ".$arrData[2];
}
...
function laporan_absensi()
{
	...
	if($data!="null")
	{
		...
		for($h=0; $h<$totalPage; $h++)
		{
			...
			{
				$output .= '<td style="padding:5px; font-size:10px;">&nbsp;</td>';
			}
			$output .= '<td style="padding:5px; text-align:right;">
						Tanggal: '.date('d-m-Y').'<br>
						Halaman: '.($h+1).'
			...
			if($h==0)
			{
				$periodeStart = indonesianDateFormat($_GET['tgl_mulai'], '-');
				$periodeEnd = indonesianDateFormat($_GET['tgl_selesai'], '-');
				$output .= '<tr align="center">';
				...
				$output .= '</tr>';
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">Periode: '.$periodeStart.' s/d '.$periodeEnd.'</td>';
				$output .= '</tr>';
			}
			$output .= '</table><br><br>';
			...
			#table header start here
			$output .= '<tr>';
			$output .= '<td rowspan="2" class="tableHeader" width="5px">No.</td>';
			$output .= '<td rowspan="2" class="tableHeader">Nama/NIP</td>';
			...
			{
				$tanggal = explode('-', $tanggal);
				$output .= '<td class="tableHeader">'.$tanggal[2].'</td>';
			}
			$output .= '<td class="tableHeader" width="20px">TPot</td>';
			$output .= '<td class="tableHeader" width="20px">Pot</td>';
			$output .= '<td class="tableHeader" width="20px">TL</td>';
			$output .= '<td class="tableHeader" width="20px">PSW</td>';
			$output .= '</tr>';
			#table header end here
			#table row start here
			for($i=0; $i<$rowPerPage; $i++)
			{
				$rowIndex = ($rowPerPage*$h) + $i;
				if($rowIndex<$totalRow)
				{
					$totalPotongan = 0;
					$totalTL = 0;
					...
					$tanpaPotongan = 0;
					$denganPotongan = 0;
					$bgColor = ($rowIndex%2==0) ? "#FFFFFF" : "#CCCCCC";
					$output .= '<tr bgcolor="'.$bgColor.'">';
					$output .= '<td class="tableRow">'.($rowIndex+1).'</td>';
					$output .= '<td class="tableRow name" width="150px" style="padding-bottom:8px;">'.$pegawaiNama[$rowIndex].'<br>'.$pegawaiNIP[$rowIndex].'</td>';
					foreach($fieldTanggal as $tanggal)
					{
						foreach($data as $value)
						{
							$absen = "";
							$tkpkn = 0;
							if($value['tanggal'] == $tanggal && $value['fsidno'] == $pegawaiNIP[$rowIndex])
							{
								...
								if( $hari != "Sun" && $hari != "Sat" && !in_array(trim($tanggal), $hariLibur) )
								{
									...
									else
									{
										...
										if ($absen!="X" || in_array($value['keterangan'], $ketTanpaPotongan) )
										{
											$tanpaPotongan++;
										}
										else
		...
		$output .= '<br><br>';
		$output .= genPenandatangan($_GET['nama'], $_GET['nip']);
	}
...
function genPenandatangan($nama, $nip)
{
	global $db;
	$output = "";
	if($nama || $nip)
	...
	{
		$sql = "SELECT b.nama, b.nip
				FROM pegawai_unitkerja a
				JOIN pegawai b ON a.nip = b.nip
				WHERE jabatan = '01010031'";
		$penandatangan = $db->dbFetchArray($sql);
		$nama = $penandatangan[0]['nama'];
		$nip = $penandatangan[0]['nip'];
		$jabatan = "Kepala Bagian Kepegawaian";
	}
	$output .= '<table cellspacing="-1" align="center" width="980px">';
	$output .= '<tr>';
	$output .= '<td width="60%">&nbsp;</td>';
	$output .= '<td width="40%">'.strtoupper($jabatan).'</td>';
	$output .= '</tr>';
	$output .= '<tr>';
	...
	$output .= '<tr>';
	$output .= '<td>&nbsp;</td>';
	$output .= '<td>'.strtoupper($nama).'</td>';
	$output .= '</tr>';
	$output .= '<tr>';
	$output .= '<td>&nbsp;</td>';
	$output .= '<td>NIP. '.$nip.'</td>';
	$output .= '</tr>';
	$output .= '</table>';
...
function daftar_absensi()
{
	...
	$end_date = date('Y-m-d', strtotime($_GET['tgl_selesai']));
	$unit_kerja = ($_GET['unit_bagian'] != 0) ? " d.unit_bagian = '".$_GET['unit_bagian']."'" : " d.unit_biro = '".$_GET['unit_biro']."'";
	$sql = "SELECT fsidno, fsname, 
				   SUM(CASE WHEN masuk IS NULL OR keluar IS NULL AND keterangan IS NULL THEN 1 ELSE 0 END) as tidakhadir,
				   SUM(CASE WHEN keterangan LIKE 'TL%' OR masuk_diff > 0 THEN masuk_diff ELSE 0 END) as tl,
				   SUM(CASE WHEN keterangan LIKE 'PSW%' OR keluar_diff > 0 THEN keluar_diff ELSE 0 END) as psw,
				   nama_id
			FROM (
			SELECT a.tanggal, b.nip as fsidno, c.nama as fsname, 
				DATE_FORMAT(FROM_UNIXTIME(a.masuk), '%H:%i:%s') as masuk,
				DATE_FORMAT(FROM_UNIXTIME(a.keluar), '%H:%i:%s') as keluar,
				a.keterangan,
				TIMESTAMPDIFF(MINUTE, CONCAT(a.tanggal, ' 07:30:00'), FROM_UNIXTIME(a.masuk)) as masuk_diff,
				TIMESTAMPDIFF(MINUTE, FROM_UNIXTIME(a.keluar), CONCAT(a.tanggal, ' 17:00:00')) as keluar_diff,
				h.v_uraian as golongan,
				(CASE WHEN d.jabatan = '-' OR d.jabatan IS NULL THEN 99999999 ELSE d.jabatan END) as jabatan,
			   (CASE WHEN g.v_uraian IS NOT NULL THEN g.v_uraian
			   WHEN f.v_uraian IS NOT NULL THEN f.v_uraian
			...
			LEFT JOIN t_par_subunitkerja g ON d.unit_subbagian = g.c_kode 
			LEFT JOIN t_par_eselon h ON d.golongan = h.c_kode 
			WHERE UNIX_TIMESTAMP(a.tanggal) >= UNIX_TIMESTAMP('2010-01-01') 
			AND UNIX_TIMESTAMP(a.tanggal) <= UNIX_TIMESTAMP('".$end_date."')
			AND ".$unit_kerja."
			AND tanggal NOT IN (SELECT tanggal FROM libur_nasional)
			AND DATE_FORMAT(tanggal, '%a') NOT IN ('Sun', 'Sat')
	...
	if($data!="null")
	{
		...
		#report style start here
		$output .= '<style>';
		$output .= '.tableHeader{font-weight:bold; border:1px solid; text-align:center; vertical-align:middle; font-size:11px;}';
		$output .= '.tableRow{border:1px solid; text-align:center; vertical-align:top; padding:2px; font-size:12px;}';
		$output .= '.name{text-align:left; font-size:12px; padding-bottom:8px;}';
		$output .= '</style>';
		#report style end here
		...
		for($i=0; $i<$totalPage; $i++)
		{
			...
			{
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">LAPORAN MONITORING KETIDAKHADIRAN PEGAWAI</td>';
				$output .= '</tr>';
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">UNIT: '.strtoupper($data[0]['nama_id']).'</td>';
				$output .= '</tr>';
				$output .= '<tr align="center">';
				$output .= '<td colspan="2">Periode: '.indonesianDateFormat($_GET['tgl_selesai'], '-').'</td>';
				$output .= '</tr>';
				$output .= '<tr>';
				$output .= '<td>Jumlah Pegawai Seluruhnya: '.$totalRow.'</td>';
				$output .= '<td>&nbsp;</td>';
			...
			$output .= '<td rowspan="2" class="tableHeader">No.</td>';
			$output .= '<td rowspan="2" class="tableHeader">Nama/NIP</td>';
			$output .= '<td rowspan="2" class="tableHeader">Tidak Hadir/X (Hari)</td>';
			$output .= '<td colspan="2" class="tableHeader">TL/PSW</td>';
			$output .= '<td rowspan="2" class="tableHeader">Total (Hari).</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td class="tableHeader">Jam</td>';
			$output .= '<td class="tableHeader">Hari</td>';
			$output .= '</tr>';
			$output .= '<tr>';
			$output .= '<td class="tableHeader">1</td>';
			$output .= '<td class="tableHeader">2</td>';
			...
			for($j=0; $j<$rowPerPage; $j++)
			{
				...
				{	
					...
					$tlpsw = $data[$rowIndex]['tl']+$data[$rowIndex]['psw'];
					$jam = floor($tlpsw/60);
					$menit = $tlpsw%60;
					$hari = floor($tlpsw/450);
					$total = $hari + $data[$rowIndex]['tidakhadir'];
					
					$output .= '<td class="tableRow">'.$jam.' jam, '.$menit.' menit</td>';
					$output .= '<td class="tableRow">'.$hari.'</td>';
					$output .= '<td class="tableRow">'.$total.'</td>';
					$output .= '</tr>';
				}
	...
	//echo $output;
	renderPDF($output, "portrait", "Daftar Absensi Pegawai");
}
...
function getDataAbsensi()
{
	...
	$output = '<table>
				<tr>
					<td>Waktu Masuk</td>
					<td>: 
						<input type="text" id="h_masuk" size="2" readonly="readonly" value="'.$masuk[0].'">:
						<input type="text" id="m_masuk" size="2" readonly="readonly" value="'.$masuk[1].'">:
						<input type="text" id="s_masuk" size="2" readonly="readonly" value="'.$masuk[2].'">
					</td>
				</tr>
				<tr>
					<td>Waktu Keluar</td>
					<td>: 
						<input type="text" id="h_keluar" size="2" readonly="readonly" value="'.$keluar[0].'">:
						<input type="text" id="m_keluar" size="2" readonly="readonly" value="'.$keluar[1].'">:
						<input type="text" id="s_keluar" size="2" readonly="readonly" value="'.$keluar[2].'">
					</td>
				</tr>
				<tr>
					<td>Keterangan</td>
...
function getDataCutiBersama()
{
	...
	$output = '<table>
				...
				<tr>
					<td colspan="2" align="center">
						<input type="button" id="btnSimpan" value="Simpan Data" onclick="simpanCutiBersama(\''.$id.'\', \''.$crudType.'\')" />&nbsp;
						<input type="button" id="btnBatal" value="Batal" onclick="$(\'#pegawaiForm\').dialog(\'close\')" />
					</td>
				</tr>
				</table>';
...
{
	global $db;
	$sql = "SELECT c_kode as id, v_uraian as nama_id FROM t_par_unitkerja WHERE c_kode LIKE '".$biro."%' AND v_uraian IS NOT NULL ";
	$result = $db->dbFetchArray($sql);
	return $result;
}
